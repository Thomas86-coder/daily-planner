...
async function run() {
  const { data, error } = await sb.from('todos').select('*').eq('done', true);
  console.log("todos count:", data?.length, "error:", error);
  const { data: pData, error: pError } = await sb.from('project_todos').select('*, projects!inner(name, category)').eq('done', true);
